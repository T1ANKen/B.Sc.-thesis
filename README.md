...
3 data_loader：读取原始数据，生成需要格式的function。

4 data_process：数据清洗、结构化，映射id等。

5 analyzer：GO数据从蛋白质到蛋白对重构，和STRING数据匹配，生成数据工程的最终文件。

6 visualizer：以上结果的可视化，包括韦恩图展示overlap等。

7 runst_batch：运行Spacedust的代码，生成的数据储存在spacedust_batch_output里。

8 query_mapper：将Spacedust的运行结果与基因的id匹配。
