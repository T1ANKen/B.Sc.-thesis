...
仓库中展现了我的全部工作代码，以下是代码文件的简要介绍。

\------------ Introduction ------------

1 main：代码的运行文件。

2 species\_config：多物种的原始数据路径，后续调用。

3 data\_loader：读取原始数据，生成需要格式的function。

4 data\_process：数据清洗、结构化，映射id等。

5 analyzer：GO数据从蛋白质到蛋白对重构，和STRING数据匹配，生成数据工程的最终文件。

6 visualizer：以上结果的可视化，包括韦恩图展示overlap等。

7 runst\_batch：运行Spacedust的代码，生成的数据储存在spacedust\_batch\_output里。

8 query\_mapper：将Spacedust的运行结果与基因的id匹配。

9 comparison：以上结果与之前的数据工程生成的文件进行比对分析、各类可视化，是分析结果的核心代码。



PS：运行pip install -r requirements.txt即可搭建复现结果的代码环境。

## data

data/raw/species文件夹是整个实验用到的大部分原始数据，包括来自GO的STRING和NCBI的数据，以及Spacedust发掘保守基因簇所得到的保守矩阵数据。数据以物种为单位分类，每个物种内有5个子文件夹，以下提供介绍。共计6个物种，C.jejuni和S.meliloti被排除。
...
faa是NCBI Assembly的官方基因组核苷酸序列文件，NCBI\_ten\_genomes是经过转换操作的核苷酸序列文件。其他信息不变，把Product\_Accesion换成Genomic\_Accesion，符合Spacedust预测工具的querydb的格式。

KEGG\_70是Spacedust的reference数据库clusterdb，作为预测工具的target数据。通过KEGG\_70和NCBI\_ten\_genomes的数据，运行runst.ipynb流程（在Linux预装好工具）即可获得输出。
...
以上提到的 **data** 与 **output** 在我的Google Drive里展示：https://drive.google.com/drive/folders/1ArZGq40EQeVjJmZZ8ohVigx\_3YmbPAtp?usp=drive\_link
